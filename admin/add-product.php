...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $pdo = getDB();
            $stmt = $pdo->prepare("INSERT INTO products (category_id, name, description, price, image, is_popular, is_daily_special, is_active, extras, keywords, gramaj, kalori, icindekiler, alerjen, bilgilendirme) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$category_id, $name, $description, $price, $imageName, $is_popular, $is_daily_special, $is_active, $extras_json, $keywords, $gramaj ?: null, $kalori ?: null, $icindekiler ?: null, $alerjen ?: null, $bilgilendirme ?: null]);
            ...
            if (isset($imageName) && file_exists(UPLOAD_PATH . $imageName)) {
...
?>
    ...
    <main class="p-4 sm:p-8 lg:p-12 max-w-7xl mx-auto w-full">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-10 gap-4">
            <div>
                <h3 class="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">Yeni Ürün Ekle</h3>
                <p class="text-slate-400 font-medium text-xs sm:text-sm mt-1">Menünüze yeni bir lezzet ekleyin.</p>
            </div>
            <div class="flex items-center space-x-2 sm:space-x-3 w-full sm:w-auto">
                ...
                <button type="submit" form="productForm" class="flex-[2] sm:flex-none bg-[var(--primary)] text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-widest shadow-xl shadow-emerald-900/10 flex items-center justify-center transition-transform active:scale-95">
                    <i class="fas fa-check-circle mr-2 sm:mr-3 opacity-60"></i> Kaydet
                </button>
            </div>
        </div>
        ...
        <form id="productForm" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div class="lg:col-span-2 space-y-6 sm:space-y-8">
                ...
                <div class="glass-card p-5 sm:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="<?= e($_POST['name'] ?? '') ?>" placeholder="Örn: Gold Caramel Latte" required class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-bold text-slate-700">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" name="price" step="0.01" min="0" value="<?= e($_POST['price'] ?? '') ?>" placeholder="0.00" required class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-bold text-slate-700">
                ...
                <div class="glass-card p-5 sm:p-8">
                    ...
                    <div class="space-y-5 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" placeholder="Ürün içeriği, hazırlanış şekli veya hikayesi..." class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-medium text-slate-600"><?= e($_POST['description'] ?? '') ?></textarea>
                        </div>
                        ...
                        <div class="pt-4 border-t border-slate-50">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                                <div>
                                    ...
                                    <input type="text" name="gramaj" value="<?= e($_POST['gramaj'] ?? '') ?>" placeholder="örn: 250 gram" class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-medium text-slate-600">
                                </div>
                                <div>
                                    <label class="block text-[9px] sm:text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2 ml-1">Kalori</label>
                                    <input type="text" name="kalori" value="<?= e($_POST['kalori'] ?? '') ?>" placeholder="örn: 320 kcal" class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-medium text-slate-600">
                                </div>
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="icindekiler" rows="3" placeholder="örn: süt, kahve, şeker..." class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-medium text-slate-600"><?= e($_POST['icindekiler'] ?? '') ?></textarea>
                                </div>
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="alerjen" rows="2" placeholder="örn: glüten, süt, yumurta..." class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-medium text-slate-600"><?= e($_POST['alerjen'] ?? '') ?></textarea>
                                </div>
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="bilgilendirme" rows="2" placeholder="örn: alkol ve domuz ürünü içermez..." class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-xs sm:text-sm font-medium text-slate-600"><?= e($_POST['bilgilendirme'] ?? '') ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="glass-card p-5 sm:p-8">
                    ...
                    <div id="noExtrasMsg" class="text-center py-8">
                ...
                <div class="glass-card p-5 sm:p-8">
                    ...
                    <div class="flex flex-col items-center">
                        <label class="w-full max-w-[200px] sm:max-w-none aspect-square bg-slate-50 rounded-[24px] sm:rounded-[32px] border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden mb-4 group relative cursor-pointer hover:border-[var(--primary)] transition-all">
                            ...
                            <div id="uploadPlaceholder" class="flex flex-col items-center group-hover:opacity-40 transition-opacity">
                            ...
                            <img id="imagePreview" src="" alt="" class="absolute inset-0 w-full h-full object-cover hidden">
                            
                            <!-- Delete Button Overlay -->
                            <button type="button" id="deleteImageBtn" class="absolute top-3 right-3 w-10 h-10 bg-rose-500 text-white rounded-xl items-center justify-center shadow-lg hover:bg-rose-600 transition-all z-20 hidden" onclick="event.preventDefault(); removeImage();">
                        ...
                        <p class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-tighter text-center">Önerilen: 800x800px (KARE)</p>
                    </div>
                </div>
